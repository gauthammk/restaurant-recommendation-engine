...
class Zomato:
    ...
    def restaurant_search(self, entity_id="", category="", collection_id="", start=0, limit=50):
        """
        Takes either entity_id, category or result limit as input.
        Returns a list of Restaurant IDs.
        """
        category = "%2C".join(category.split(","))
        if str(limit).isalpha() is True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?count=" + str(limit) + "&start=" + str(start) + "&entity_id=" +
                          str(entity_id) + "&entity_type=city" + "&category=" + str(category) + "&collection_id=" + str(collection_id) + "&sort=rating&order=desc", headers=headers).content).decode("utf-8")
        a = json.loads(r)
        ...
        if a['results_found'] == 0:
            ...
            print('results_found : ', a['results_found'])
            for restaurant in a['restaurants']:
                restaurant_ids.append(restaurant['restaurant']['id'])
    ...
    def is_valid_restaurant_id(self, restaurant_ID):
        ...
        if restaurant_ID.isnumeric() is False:
